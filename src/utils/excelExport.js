...
const STYLES = {
  ...
  title: {
    font: { bold: true, sz: 14, color: { rgb: "1C1C2E" } },
    fill: { fgColor: { rgb: "F5F3EE" } },
    alignment: { horizontal: "center" },
  },
  label: {
    font: { sz: 10, color: { rgb: "4A4A6A" } },
    border: borders("thin"),
  },
  value: {
    font: { bold: true, sz: 11 },
  ...
  warning: {
    font: { sz: 10, color: { rgb: "A16207" } },
    fill: { fgColor: { rgb: "FEFCE8" } },
  },
  sectionBg: {
    fill: { fgColor: { rgb: "F0EDE6" } },
    font: { bold: true, sz: 11 },
  },
};
...
function numCell(value, style = {}) {
  return {
    v: typeof value === "string" ? parseFloat(value) || 0 : value,
    t: "n",
    s: style,
  };
}
...
class SheetBuilder {
  ...
  setCell(col, row, val, style) {
    this.data[`${col}${row}`] =
      typeof val === "number" ? numCell(val, style) : cell(val, style);
  }

  addMerge(s, e) {
    this.merges.push({ s, e });
  }
  ...
  addRow(label, value, unit = "", pass = null) {
    ...
    this.data[`D${this.row}`] =
      typeof value === "number"
        ? numCell(value, STYLES.value)
        : cell(String(value), STYLES.value);
    this.data[`E${this.row}`] = cell(unit, STYLES.label);
    if (pass !== null) {
      this.data[`F${this.row}`] = cell(
        pass ? "✓ PASS" : "✗ FAIL",
...
function buildCoverSheet(projectInfo) {
  ...
  [
    ...
    "Bridge Loads",
    "Estimation & BOQ",
  ].forEach((m) => sb.addRow(m, "Included", ""));

  sb.addBlankRow();
  sb.data[`A${sb.row}`] = cell(
    "DISCLAIMER: This report is for preliminary design and educational purposes only. All results must be verified by a licensed structural/civil engineer.",
    STYLES.warning,
  );
...
function buildBeamSheet(data) {
  ...
  data.loads.forEach((l, i) => {
    ...
    if (l.type === "point")
      sb.addRow(`Load ${i + 1} — Point @ ${l.pos}m`, l.p, "kN");
  });

  sb.addSectionHeader("ANALYSIS RESULTS");
  sb.addRow(
    "Reaction at A (Ra)",
    parseFloat(data.result?.Ra?.toFixed(2) || 0),
    "kN",
  );
  sb.addRow(
    "Reaction at B (Rb)",
    parseFloat(data.result?.Rb?.toFixed(2) || 0),
    "kN",
  );
  sb.addRow(
    "Max Shear Force",
    parseFloat(data.result?.maxV?.toFixed(2) || 0),
    "kN",
  );
  sb.addRow(
    "Max Bending Moment",
    parseFloat(data.result?.maxM?.toFixed(2) || 0),
    "kNm",
  );
...
function buildColumnSheet(data) {
  ...
  if (data.result?.isSlender)
    sb.addRow("Warning", "Additional moment check required (IS 456 Cl. 39.7)");
...
function buildSlabSheet(data) {
  ...
  sb.addRow(
    "Limiting Moment (Mulim)",
    parseFloat(data.result?.Mulim || 0),
    "kNm/m",
  );
...
function buildRoadSheet(data) {
  ...
  sb.addRow(
    "Min Radius Check",
    data.hCurve?.passRadius ? "PASS" : "FAIL",
    "",
    data.hCurve?.passRadius,
  );
  ...
  sb.addRow(
    "Vertical Curve Check",
    data.vCurve?.pass ? "PASS" : "FAIL",
    "",
    data.vCurve?.pass,
  );
...
function buildBOQSheet(data) {
  ...
  // BOQ table header
  const hRow = sb.row;
  ["#", "Description", "Unit", "Qty", "Rate (₹)", "Amount (₹)"].forEach(
    (h, i) => {
  ...
  (data.boqItems || []).forEach((item, idx) => {
    ...
    rowVals.forEach((v, i) => {
      sb.data[`${cols[i]}${sb.row}`] =
        typeof v === "number"
          ? numCell(v, STYLES.value)
          : cell(String(v), STYLES.label);
    });
    sb.row++;
  });

  sb.row++;
  sb.data[`A${sb.row}`] = cell("Grand Total", {
    font: { bold: true, sz: 12 },
    fill: { fgColor: { rgb: "ECFDF5" } },
  });
  sb.addMerge({ r: sb.row - 1, c: 0 }, { r: sb.row - 1, c: 4 });
  sb.data[`F${sb.row}`] = numCell(total, {
    font: { bold: true, sz: 13, color: { rgb: "059669" } },
    alignment: { horizontal: "right" },
  });
  sb.row++;
...
export function exportToExcel(allModuleData) {
  ...
  // Module sheets
  if (allModuleData.beam)
    XLSX.utils.book_append_sheet(
      wb,
      buildBeamSheet(allModuleData.beam),
      "Beam Design",
    );
  if (allModuleData.column)
    XLSX.utils.book_append_sheet(
      wb,
      buildColumnSheet(allModuleData.column),
      "Column Design",
    );
  if (allModuleData.slab)
    XLSX.utils.book_append_sheet(
      wb,
      buildSlabSheet(allModuleData.slab),
      "Slab Design",
    );
  if (allModuleData.foundation)
    XLSX.utils.book_append_sheet(
      wb,
      buildFoundationSheet(allModuleData.foundation),
      "Foundation",
    );
  if (allModuleData.road)
    XLSX.utils.book_append_sheet(
      wb,
      buildRoadSheet(allModuleData.road),
      "Road Design",
    );
  if (allModuleData.boq)
    XLSX.utils.book_append_sheet(wb, buildBOQSheet(allModuleData.boq), "BOQ");
